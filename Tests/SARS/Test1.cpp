...
int main()
{
	...
	Question r21("V stila clien t-server komponentite vzaimodeystvat pomejdu si chrez izprashtane na syobshteniq(ili izvikvane na metod) pri koeto izprashtashtiq komponent ne znae nishto za poluchatelq?\nIzberete edno");
	...
	Question q1("Koi ot slednite sa primer sa dobre formuliran scenarii za kachestvo? 1 ili poweche");
	q1.AddOption("Dopustimo e sistemata da e w revim na poddrujka za ne poweche ot 3chasa sedmichno", 1);
	q1.AddOption("Potrebitelqt wliza w sistemata, izbira opciqta za tursene I vuvejda operedelen simwolen niz", -1);
	q1.AddOption("Promeni w sistemata da se prawqt wyzmovno nai-byrzo", -1);
	q1.AddOption("Otgoworut na potrebitelskite zaqvki trqbva da stane do 30 sec", 1);
	exam.push_back(q1);

	Question q2("Smisulut na nefykcionalnite iziskvaniq kum sistemata e");
	q2.AddOption("Da se ylesni razrabotkata na sistemata", -1);
	...
	q4.AddOption("Vuzmojnost na sistemata da osigyri rabota bez otkaz", -1);
	...
	q7.AddOption("se proverqva dali proektiranta veche architektyra udovletvorqva postavenite iziskvaniq", -1);
	exam.push_back(q7);

	Question q8("Pri kakuv tip sistemi e nai-podhodqshto da se izpolzva stryktyrata na vnedrqvaneto");
	q8.AddOption("Koito se izpylnqwat na edna mashina", -1);
	...
	q10.AddOption("ogranichi neobhodimostta ot promeni s drugi moduli, s koito dadeniqt komynikira", 1);
	q10.AddOption("namali slovnostta na sistemata I da se yskori procesa na razrabotka", -1);
	q10.AddOption("naprawi arhitekturata po-qsna I da se razbira ot wsichki zainteresowani lica", -1);
	q10.AddOption("ne se prilaga takawa taktika", -1);
	exam.push_back(q10);

	Question q11("Stuktura na procesite :");
	q11.AddOption("Posochwa zaduljeniqta na otdelnite zainteresowani lica", -1);
	q11.AddOption("Pokazwa posledovatelnostta na izpulnenie na otdelnite komponenti v sistemata", 1);
	q11.AddOption("E izgotwen ot menidjura na proekta dokument, koito naznachawa zadachite na ostanlite ychastnici w razrabotkata na proekta", -1);
	q11.AddOption("Pokazwa abstraktni grupi ot hora, koito realizirat modulite na sistemata", -1);
	exam.push_back(q11);
	...
	Question q13("W softuernata arhitektura e redno da se opiswat swoistwa I metodi na klasowete, koito izgrajdat obektite w sistemata");
	q13.AddOption("True", -1);
	q13.AddOption("False", 1);
	exam.push_back(q13);

	Question q14("V koi ot slednite deinosti e redno da wzimat ychastie softuernite arhitekti ? Edno ili poweche");
	q14.AddOption("Opredelqne na algoritmite za razabotka na sistemata", -1);
	q14.AddOption("Razrabotka na sistemata", 1);
	q14.AddOption("Ytochnqwane na iziskwaniqta kym softuernata sistema", 1);
	q14.AddOption("Proektirane na arhitektyrata", 1);
	q14.AddOption("Izgotwqne na biznes plan na proekta", -1);
	exam.push_back(q14);
